          ...
          <div className="flex flex-col justify-between p-7 lg:col-span-5 lg:p-10">
            <div>
              <div className="flex items-baseline gap-2">
                <span className="font-display text-4xl font-bold text-brand-900">
                  {formatNaira(p.price)}
                </span>
                {period && (
              ...
              <div className="mt-5 flex flex-wrap gap-3 border-t border-ink-100 pt-5 text-sm text-ink-700">
                {p.bedrooms != null && (
                  <span className="flex items-center gap-1.5 rounded-full bg-brand-50 px-3 py-1.5 text-xs">
                    <Bed size={13} className="text-brand-700" />
                    {p.bedrooms} Beds
                  </span>
                )}
                ...
                {p.bathrooms != null && (
                  <span className="flex items-center gap-1.5 rounded-full bg-brand-50 px-3 py-1.5 text-xs">
                    <Bath size={13} className="text-brand-700" />
                    {p.bathrooms} Baths
                  </span>
                )}
                ...
                {p.area_sqm != null && (
                  <span className="flex items-center gap-1.5 rounded-full bg-brand-50 px-3 py-1.5 text-xs">
                    <Maximize size={13} className="text-brand-700" />
                    {p.area_sqm} sqm
                  </span>
                )}
              </div>
            ...
            <div className="mt-7 flex flex-wrap gap-2">
              <a
                ...
                className="inline-flex items-center gap-1.5 rounded-full bg-brand-700 px-5 py-2.5 text-xs font-semibold text-white"
              >
                <Phone size={12} />
                Call Agent
              </a>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 rounded-full border border-ink-200 bg-white px-5 py-2.5 text-xs font-semibold text-ink-800 hover:bg-ink-50"
              >
  ...
  if (layout === 'wide') {
    return (
      <article className="group flex flex-col overflow-hidden rounded-3xl border border-ink-100 bg-white transition hover:border-brand-200 hover:shadow-lg sm:flex-row">
        <div className="relative aspect-[4/3] overflow-hidden bg-ink-100 sm:aspect-auto sm:w-72 sm:shrink-0">
          <PropertyImageGallery
            images={p.images}
            title={p.title}
            sizes="288px"
            badge={
              <span className="absolute left-3 top-3 rounded-full bg-white/95 px-3 py-1 text-[10px] font-semibold text-ink-900 backdrop-blur">
                {intent}
              </span>
            }
          />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-ink-100 pt-4 text-xs text-ink-700">
            {p.bedrooms != null && (
              <span className="flex items-center gap-1">
                <Bed size={12} className="text-brand-700" />
                {p.bedrooms}
              </span>
            )}
            ...
            {p.bathrooms != null && (
              <span className="flex items-center gap-1">
                <Bath size={12} className="text-brand-700" />
                {p.bathrooms}
              </span>
            )}
            ...
            {p.area_sqm != null && (
              <span className="flex items-center gap-1">
                <Maximize size={12} className="text-brand-700" />
                {p.area_sqm} sqm
              </span>
            )}
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="ml-auto inline-flex items-center gap-1 rounded-full bg-brand-700 px-3 py-1.5 text-[11px] font-semibold text-white"
            >
    ...
    <article className="group block overflow-hidden rounded-3xl border border-ink-100 bg-white transition hover:border-brand-200 hover:shadow-xl hover:shadow-brand-900/5">
      ...
      <div className="p-6">
        <div className="flex items-baseline gap-2">
          ...
          {period && (
            <span className="text-sm font-medium text-ink-500">{period}</span>
          )}
        </div>
        ...
        <div className="mt-4 flex items-center gap-4 border-t border-ink-100 pt-4 text-xs text-ink-700">
          ...
          {p.bathrooms != null && (
            <span className="flex items-center gap-1.5">
              <Bath size={14} className="text-brand-700" />
              {p.bathrooms} Baths
            </span>
          )}
          ...
          {p.area_sqm != null && (
            <span className="flex items-center gap-1.5">
              <Maximize size={14} className="text-brand-700" />
              {p.area_sqm} sqm
            </span>
          )}
        </div>
          ...
          <div className="relative mx-auto max-w-7xl px-6 py-16 lg:py-20">
            ...
            <RevealOnScroll delay={0.2}>
              <p className="mt-6 max-w-2xl text-lg text-ink-600">
                We act as agent for vendors and landlords across residential,
                commercial and industrial property. Every listing has been
                appraised by our valuers — so you know the asking price reflects
                the real market.
              </p>
            </RevealOnScroll>
                  ...
                  <div className="grid lg:grid-cols-2">
                    ...
                    <div className="p-8 lg:p-12">
                      ...
                      <p className="mt-3 text-base leading-relaxed text-ink-600">
                        We may not have a current public listing that matches
                        your search — but we have an active network of vendors
                        and landlords. Tell us what you&apos;re looking for and
                        ...
                        {[
                          {
                            icon: Home,
                            t: 'Residential',
                            d: 'Bungalows, duplexes, blocks of flats, serviced apartments.',
                          },
                          {
                            icon: Building2,
                            t: 'Commercial',
                            d: 'Offices, retail, hotels, mixed-use developments.',
                          },
                          {
                            icon: Briefcase,
                            t: 'Industrial',
                            d: 'Warehouses, factories, logistics facilities.',
                          },
                          {
                            icon: Tag,
                            t: 'Land',
                            d: 'Bare land for development, agricultural, or investment.',
                          },
                        ].map((c) => (
                          <div
                            key={c.t}
                      ...
                      <div className="mt-8 flex flex-wrap gap-3">
                        <Link
                          href="/contact"
                          className="inline-flex items-center gap-1.5 rounded-full bg-brand-700 px-5 py-2.5 text-sm font-semibold text-white"
                        >
                          <Search size={14} />
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1.5 rounded-full border border-ink-200 bg-white px-5 py-2.5 text-sm font-semibold text-ink-800 hover:bg-ink-50"
                        >
                ...
                <RevealOnScroll>
                  <div className="rounded-3xl bg-brand-50/40 p-8 text-center ring-1 ring-brand-100 sm:p-12">
                    ...
                    <p className="mx-auto mt-2 max-w-xl text-sm text-ink-600">
                      Our agency network covers many off-market properties. Let
                      us know your specific brief.
                    </p>

                    <Link
                      href="/contact"
                      className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-brand-700 px-5 py-2.5 text-sm font-semibold text-white"
                    >
                      <Search size={14} />
                      Send a Brief
                    </Link>
                ...
                <RevealOnScroll>
                  <div className="mt-8 rounded-3xl bg-brand-50/40 p-8 text-center ring-1 ring-brand-100">
                    <h3 className="font-display text-xl font-bold text-ink-900">
                      Want something specific? Tell us.
                    </h3>

                    <Link
                      href="/contact"
                      className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-brand-700 px-5 py-2.5 text-sm font-semibold text-white"
                    >
                      <Search size={14} />
                      Send a Brief
                    </Link>
                ...
                <div className="grid items-center gap-8 lg:grid-cols-12 lg:gap-16">
                  <div className="lg:col-span-7">
                    ...
                    <p className="mt-4 text-base text-ink-300">
                      We act for vendors and landlords across Nigeria — properly
                      priced, professionally marketed, with vetted enquiries
                      only.
                    </p>
                  </div>
